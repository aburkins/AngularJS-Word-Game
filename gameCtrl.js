 ...
 */
            ...
            app.controller("gameInitCtrl", function($scope){
                var num = getRandomIntInclusive(0,wordbank.length-1);
                var answer = wordbank[num].split("");
                //console.log(answer);
                //the chosen word as an array
                $scope.answer = answer;
                //player's progress on screen as an array
                $scope.displayArray = [];
                $scope.numTries = 10;
                $scope.guesses = [];
                $scope.guessesDisplay = "";
                $scope.gameOver = false;
                $scope.gameWon = false;
         
                for(var i = 0; i < answer.length; i++){
                    $scope.displayArray[i] = "_";
                }
                //players progress as displayed on screen
                $scope.display = getArrayStr($scope.displayArray);
                
                //called on key-up in input box
                $scope.processGuess = function(guess,formValid){
                    //console.log("executing processGuess");
                    
                    if(guess && formValid && !$scope.gameOver){
                        //console.log(guess);
                        var word = $scope.answer;
                        var found = false;
                        for(var i = 0; i < word.length; i++){
                            if(word[i] === guess){
                                //update the display
                                $scope.displayArray[i] = word[i];
                                found = true;
                            }
                        }
                        //if not already guessed:
                        if($scope.guesses.indexOf(guess) === -1){
                            if(!found) --$scope.numTries;
                            if($scope.numTries === 0){
                                $scope.gameOver = true;
                                $scope.displayArray = $scope.answer;
                            }
                            //update the guesses array
                            $scope.guesses.push(guess);
                        }
                        
                        if($scope.displayArray.isEqual($scope.answer) && !$scope.gameOver){
                            $scope.gameOver = true;
                            $scope.gameWon = true;
                        }
                        //update displays
                        $scope.display = getArrayStr($scope.displayArray);
                        $scope.guessesDisplay = getArrayStr($scope.guesses);
                        
                        
                        clearInput();
                    }
                    //else if($scope.gameOver) console.log("game is over.");
            ...
            //tests for arrays with all equal values
            Array.prototype.isEqual = function(testArray){
                if(this.length !== testArray.length) return false;
                for(var i = 0; i < this.length; i++){
                    if(testArray[i] !== this[i]) return false;
                }
                return true;
            };
